...
app.post('/file/new', function (req, res) {
    ...
    let ownerID = req.body.ownerID
    let fileSize = fileToSplit.data.length/1024; // File size in KB.
    console.log('Uploaded ' + fileName + ' size: ' + fileSize);
                                    ...
                                    if (err)
                                        ...
                                        fileHelper.findHoldersForChunks(chunksNames, ownerID, function (err, chunksWithHolderID) {
                                            if (err)
                                                ...
                                                db.newFile(fileName, chunksWithHolderID, ownerID, fileSize, function (err) {
                                                    if (err) {
...
app.post('/holder/create', function (req, res) {
    ...
    db.createHolder(ownerID, bytesAvailable, holderName, function (err) {
        if (err)
